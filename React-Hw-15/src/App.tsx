...
function App() {
  ...
  const MinusOnclickHandler = (index: number) => {
    setCafeData(prevCafeData => {
      ...
      setTotal(prevTotal =>
        prevTotal > 0 && prevCafeData[index].quantity > 0
          ? prevTotal - prevCafeData[index].price
          : prevTotal,
      );
  ...
  return (
    <div className="container max-w-[1000px]  mx-auto ">
      <div className="bg-[#ececec] pb-14">
        ...
        <p className="w-fit mx-auto py-4 font-bold text-lg  text-slate-800">
        ...
        <p className="w-fit mx-auto py-4 font-bold text-lg text-slate-800 hidden md:block">
          Bill
        </p>
          ...
          {data
            ? CafeData.map((card, i) => {
                return (
                  <div
                    key={i}
                    className="bg-[#1c3a32] flex flex-col max-w-40 rounded-lg  p-3 gap-y-3"
                  >
                    <img src={card.img} alt="" className="w-40 mx-auto" />
                    <p className="font-semibold text-white text-left">
                      {card.name}
                    ...
                    <p className="text-[#e7c7a1] font-extrabold">
                      ${Number(card.price) * Number(card.quantity)}
                    </p>
        ...
        <div className="w-full p-4 flex justify-center">
          <button
            ...
            className="font-bold text-white bg-[#ddb078] w-full max-w-[920px] p-2 text-xl rounded-md hover:bg-[#e1b682]"
          >
            Submit Order
          </button>
        </div>
